...
class OutreachColumnContractTests(unittest.TestCase):
    ...
    def test_url_picker_worker_picks_retryable_and_stale_enrichment_rows(self):
        ...
        self.assertIn("(status_reason,eq,skipped_url_validation_failed)", url_expr)
        self.assertNotIn("const sendSafeWhere = '(send_provider,neq,instantly)", url_expr)
    ...
    def test_url_picker_worker_patches_transport_timeout_as_retryable(self):
        ...
        self.assertIn("text.includes('bad gateway')", enrichment_code)
...
class InstantlyWorkflowContractTests(unittest.TestCase):
    def test_automation_controller_wakes_worker_and_planner_when_enabled(self):
        ...
        self.assertIn("RAYN_AUTOMATION_CONTROLLER_PHASE", js_code)
        ...
        self.assertIn("explicitTicks", js_code)
        self.assertIn("copy_qa_mode: copyQaMode", js_code)
        self.assertIn("RAYN_AUTOMATION_WORKER_TICKS', 1, 10", js_code)
        self.assertIn("RAYN_CONTACT_SEARCH_BATCH_LIMIT, 10, 1, 10", js_code)
