...
**Plans**:
1. **Railway env var hardening** — Set `N8N_CONCURRENCY_PRODUCTION_LIMIT=1`, `N8N_PAYLOAD_SIZE_MAX=128`, `N8N_RUNNERS_TASK_TIMEOUT=600` on n8n service; set `DB_QUERY_LIMIT_MAX=10000`, `DB_QUERY_LIMIT_DEFAULT=1000` on NocoDB service; validate actual row count vs. n8n Get All output today (covers INFRA-01, INFRA-02)
2. **Race condition status lock** — Implement pessimistic status lock in wf-latest: fetch IDs only → immediately `UPDATE status="processing"` → fetch full record → enrichment chain; add stuck-processing cleanup that resets rows older than 10 min back to `pending` (covers FIX-01)
3. **Batch processing + OpenRouter backoff** — Replace `.first()` pattern with Loop Over Items (Split in Batches, batch size 1) so all 5 filtered leads are processed per run; add exponential backoff (1s → 2s → 5s → 13s) on all OpenRouter HTTP Request nodes before enabling 5x throughput (covers FIX-02, FIX-03)
4. **Contact fallback + email validation timeout** — Change IF No Contact condition from AND to OR so Hunter is triggered when name OR email is empty; redesign No2Bounce polling as a separate scheduled workflow using Wait nodes instead of inline polling, writing `verification_timeout` status after 6 retries and allowing retry from NocoDB (covers FIX-04, FIX-05)
5. **NocoDB pagination in both workflows** — Replace hardcoded `limit=10000` GET calls with `pageInfo.isLastPage` pagination loops in wf-latest (all pending-lead fetches) and wf-discovery (Read Discovery + Read Leads fetches); add in-batch URL hash dedup in wf-discovery before NocoDB write (covers FIX-06, FIX-07)
...
|-------|----------------|--------|-----------|
| 1. Workflow Reliability | 0/5 | Not started | - |
